...
% del atlas los produce docs/segunda_entrega/make_atlas_figs.jl y se toman de
% ahí para no duplicar ni el cálculo ni los archivos.
\graphicspath{{figs/}{../../segunda_entrega/figs/}}
...
de álgebra lineal. Quinto, todo queda blindado por 597 pruebas de regresión que
...
La primera entrega recorrió, para dos configuraciones, la cadena completa
\[
\text{Euler--Lagrange} \to \Mm(q)\ddot q=\text{rhs} \to \text{linealización}
\to (A,B,C) \to \spec(A),\ \rank\calC,\ \rank\calO \to K \to \text{simulación}.
\]
...
\textbf{$N=1$, barra uniforme} ($a_1=L$, $I_1=\tfrac{1}{12}m(2L)^2$, sin
eslabones superiores): $\beta_1=mL$ y $J_1=I+mL^2$. La ecuación del carro queda
...
$\lambda=0$ con multiplicidad dos, en un bloque de Jordan $2\times2$.
\end{proof}
...
Los rangos de Kalman son plenos, $\rank\calC=\rank\calO=8$, y el LQR con
$Q=\diag(1,0,10,0,10,0,10,0)$ y $R=0.1$ ---los mismos pesos de las otras dos
configuraciones, extendidos--- da
\[
...
$\rank\calC=n$ para $\calC=[B\ \ AB\ \cdots\ A^{n-1}B]$. Es un teorema correcto.
El problema es que $\calC$ es una \emph{base de Krylov}, notoriamente mal
condicionada: sus columnas tienden a alinearse con el eigenvector dominante de
$A$ a medida que crece la potencia.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}cccccc@{}}
    ...
    4 & 10 & 26.54 & $1.91\times10^{-4}$ & $1.7\times10^{12}$ & 5266.8\\
...
Esto ya no es binario. Por el teorema de Eckart--Young, $\sigma_{\min}$ de una
matriz es exactamente su distancia en norma 2 al conjunto de matrices de rango
deficiente; por tanto $\mu_{\text{PBH}}$ mide \emph{cuánto habría que perturbar
...
verdadera distancia a la incontrolabilidad de Eising,
...
Con $V(\xv)=\xv^\top P\xv$ la función de Lyapunov del LQR y $\uv=-K\xv$, el
mayor conjunto de nivel $\calE_c=\{\xv:\xv^\top P\xv\le c\}$ contenido en la
franja de no saturación $\{|K\xv|\le u_{\max}\}$ se obtiene resolviendo
\[
...
además, acotar el residuo de la linealización. Presentar $c^\star$ como si fuera
...
Van Loan: construyendo la matriz aumentada de tamaño $(n+m)\times(n+m)$
\[
...
porque el carro aporta un eigenvalor nulo. La implementación reproduce
$e^{Ah}$ y la integral a precisión de máquina: el error contra una serie de
Taylor de orden 20 en aritmética de precisión extendida es $0$ en $A_d$ y
$3.5\times10^{-18}$ en $B_d$.
...
unitario. Es el análogo discreto de Hurwitz, y sale de que la solución discreta
es $\xv_k=(A_d-B_dK)^k\xv_0$.
...
\begin{enumerate}
  ...
  \item \textbf{Más actuador deja de comprar ángulo a partir de unos 53~N.} La
  frontera se aplana en $0.253$~rad. Comprar un motor más grande no mejora la
  región de operación.
\end{enumerate}
...
Los barridos geométricos confirman una intuición conocida y la cuantifican: el
eslabón superior \emph{corto} es el peligroso. Con $\ell_3=2$~cm el modo
dominante sube a $42.9$~s$^{-1}$ y el margen PBH cae a $3.4\times10^{-5}$;
alargarlo hasta 2~m lo baja a $17.3$~s$^{-1}$ y sube el margen a
$6.0\times10^{-4}$. Es el efecto de la escoba larga en la palma de la mano, ahora
medido y en la punta de un péndulo triple.
...
real eso no se mide. Se estima con
\[
...
aumentado. No dice nada sobre la región de atracción del sistema no lineal, y
ahí el precio de estimar en vez de medir resulta considerable. Midiendo por
...
La caída es sustancial: entre la mitad y la cuarta parte. El mecanismo es claro
en la simulación: durante los primeros instantes el controlador actúa sobre una
estimación equivocada ---el pico de fuerza sube de 7.3~N a 26~N--- y ese esfuerzo
mal dirigido saca a la planta del régimen en que la linealización es válida. Una
vez fuera, el observador lineal ya no describe bien la planta y el error deja de
converger.
...
Una consecuencia práctica: el pico de fuerza con observador es de 26~N frente a
7.3~N con estado completo, porque durante el primer medio segundo el controlador
actúa sobre una estimación equivocada. Al dimensionar el actuador hay que contar
con ese transitorio.
...
\textbf{Pruebas de regresión.} El repositorio incluye 597 pruebas organizadas en
...
\begin{enumerate}
  ...
  el valor previo provenía de una malla gruesa en la que $0.1$ era un nodo.
...
\texttt{test/runtests.jl} & 597 pruebas de regresión\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{@{}ll@{}}
\toprule
\texttt{julia --project=. main\_triple.jl} & pipeline y figuras del triple\\
\texttt{julia --project=. -t auto docs/segunda\_entrega/make\_atlas\_figs.jl} & atlas\\
\texttt{julia --project=. -t auto .../entrega\_2/make\_report\_figs.jl} & figuras del informe\\
\texttt{julia --project=. test/runtests.jl} & pruebas\\
\bottomrule
\end{tabular}
\end{center}
