...
\newcommand{\Gm}{\bm{G}}
\newcommand{\Fm}{\bm{F}}
% La matriz de amortiguamiento se denota D y no F: F ya designa la matriz de
% entrada en la forma mecanica estandar M(q) qdd + C qd + G = F u.
\newcommand{\Dm}{\bm{D}}
...
\begin{center}
    ...
    {\LARGE\bfseries El Límite No Es Teórico, Es Numérico}\\[0.5em]
...
órdenes de magnitud por eslabón y con cinco rebasa $1/\varepsilon_{\text{máq}}$.
...
\end{gathered}
\]
El resultado fue sólido, pero un lector crítico podía señalar cinco huecos de
fondo. Esta segunda entrega los ataca uno por uno.
...
escala sin grandes cambios conceptuales al crecer el número de eslabones de la
cadena, y con él la dimensión del estado y el grado de subactuación; pero
\emph{el condicionamiento no}. Llega un punto en que los teoremas siguen
siendo ciertos y los algoritmos que los implementan dejan de ser confiables. La
...
% ============================================================
\section{Formalización genérica para un número arbitrario de eslabones}
\label{sec:generico}
...
\begin{equation}
\Mm_0\,\ddot q=\Gm_0\,q-\Dm_0\,\dot q+\bm{e}_1u,
\label{eq:linealizado}
\end{equation}
con $\Mm_0=\Mm(0)$, $\Gm_0=g\diag(0,\beta_1,\dots,\beta_N)$ y $\Dm_0$ la matriz
de fricción. Despejando se obtienen los tres bloques
\[
A_{cc}=\Mm_0^{-1}\Gm_0,\qquad A_{cv}=-\Mm_0^{-1}\Dm_0,\qquad
B_c=\Mm_0^{-1}\bm{e}_1,
\]
...
\label{prop:espectro}
Si $\Dm_0=0$, los eigenvalores de $A$ vienen en pares $\pm\lambda$ más un cero
doble:
\[
...
\cref{eq:linealizado} con $\Dm_0=0$ y $u=0$ se obtiene el problema generalizado
...
\begin{figure}[htbp]
  ...
  1.84, 1.86 y 1.88~s con un pico de fuerza de 7.29~N.}
...
\begin{alerta}
\small
\textbf{Con $N=5$ el condicionamiento rebasa siete veces
$1/\varepsilon_{\text{máq}}$.} En
...
\begin{figure}[htbp]
  ...
  \caption{Izquierda: el condicionamiento de la matriz de Kalman crece casi
  quince órdenes de magnitud entre $N=1$ y $N=5$, cruzando el umbral de
  $1/\varepsilon_{\text{máq}}$. Derecha: el margen PBH decae suavemente, poco
  más de dos órdenes ---un factor $145$---, y sigue siendo utilizable.}
...
integral a precisión de máquina: contra una serie de Taylor de orden 20 evaluada
con 256 bits, la mayor discrepancia en la Configuración~III con $h=10$~ms es de
$7.7\times10^{-16}$ en $A_d$ y $3.0\times10^{-18}$ en $B_d$, del orden del
redondeo de la doble precisión y no un error del método.
...
\begin{enumerate}
  ...
  punto.} Para $u_{\max}<85$~N queda por dentro de la frontera numérica, como
...
Kalman, $(A,C)$ observable $\iff$ $(A^\top,C^\top)$ controlable, de modo que
\begin{equation}
...
observador ($0.0743$~rad): con estado completo los tres ángulos se asientan en
1.88~s, con observador diverge. Es un recordatorio de que el principio de
separación es un
...
\begin{enumerate}
  ...
  $0.22\%$--- pero conviene decirlo: un óptimo leído de una malla siempre hereda
  su resolución.
...
\begin{enumerate}
  ...
  valiendo para cualquier $N$, pero $\operatorname{cond}\calC$ crece cerca de
  cuatro órdenes por eslabón ---cada vez más--- y con cinco rebasa el límite de
...
\texttt{-t auto docs/Entrega\_2/informe\_tecnico/make\_report\_figs.jl} & figuras del informe\\
...
\small
% Las once entradas caben en la ultima pagina del cuerpo si se aprieta el
% espaciado entre items; sin esto la bibliografia se derrama a una pagina
% adicional que solo lleva dos referencias.
